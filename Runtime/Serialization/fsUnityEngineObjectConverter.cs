...
namespace Devdog.General
{
    public class fsUnityEngineObjectConverter : fsConverter
    {
        ...
        {
            if (typeof(IAsset).IsAssignableFrom(type)) return true;
        ...
        {
            if (IsAssetWrapper(type)) return true;
...
//            }
        ...
        protected virtual Object GetUnityEngineObject(object instance)
        {
            Object unityObject = null;
            var iAsset = instance as IAsset;
            if (iAsset != null) unityObject = iAsset.objectVal;

            if (iAsset == null) unityObject = instance as Object;
        ...
        {
            var db = Serializer.Context.Get<List<Object>>();

            var unityObject = GetUnityEngineObject(instance);
            var index = db.IndexOf(unityObject);
            if (index == -1)
            {
        ...
        public override fsResult TryDeserialize(fsData data, ref object instance, Type storageType)
        {
            try
            {
                ...
                if (data.IsNull == false)
                {
                    ...
                    {
                        DevdogLogger.LogError("Couldn't deserialize UnityEngine.Object : " + instance +
                                              " - not found in database. (index: " + index + ")");
                    ...
                    else if (typeof(Object).IsAssignableFrom(storageType))
                    {
                        instance = db[index];
                    }
                }
