    ...
    public class fsMetaType
    {
            ...
            get
            {
                if (_hasDefaultConstructorCache.HasValue == false)
                {
                    ...
                    else
                    {
                        ...
                        if (ctor != null) _isDefaultConstructorPublic = ctor.IsPublic;
                    }
                }
        ...
        private static void CollectProperties(fsConfig config, List<fsMetaProperty> properties, Type reflectedType)
        {
            ...
            foreach (var member in members)
            {
                ...
                if (config.IgnoreSerializeAttributes.Any(t => fsPortableReflection.HasAttribute(member, t))) continue;

                var property = member as PropertyInfo;
                var field = member as FieldInfo;

                // Early out if it's neither a field or a property, since we don't serialize anything else.
                if (property == null && field == null) continue;

                // Skip properties if we don't want them, to avoid the cost of checking attributes.
                if (property != null && !config.EnablePropertySerialization) continue;
            ...
            bool annotationFreeValue)
        {
            ...
            if (config.SerializeAttributes.Any(t => fsPortableReflection.HasAttribute(property, t))) return true;
            ...
            if (property.CanRead == false || property.CanWrite == false) return false;
        ...
        private static bool CanSerializeField(fsConfig config, FieldInfo field, bool annotationFreeValue)
        {
            ...
            if (field.IsDefined(typeof(CompilerGeneratedAttribute), false)) return false;
            ...
            //       static fields.
            if (config.SerializeAttributes.Any(t => fsPortableReflection.HasAttribute(field, t))) return true;

            // We use !IsPublic because that also checks for internal, protected, and private.
            if (!annotationFreeValue && !field.IsPublic) return false;
        ...
        public object CreateInstance()
        {
            ...
            if (typeof(string) == ReflectedType) return string.Empty;
