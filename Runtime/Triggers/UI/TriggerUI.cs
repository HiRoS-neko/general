...
namespace Devdog.General.UI
{
    ...
    public class TriggerUI : MonoBehaviour
    {
        ...
        {
            if (oldPlayer != null) oldPlayer.triggerHandler.OnSelectedTriggerChanged -= BestTriggerChanged;
        ...
        private void BestTriggerChanged(TriggerBase old, TriggerBase newBest)
        {
            if (newBest != null)
            {
                _window.Show();
                Repaint(newBest);
                if (moveToTriggerLocation) UpdatePosition(newBest);
            }
            else
            {
                _window.Hide();
        ...
        protected virtual Sprite GetIcon(ITriggerInputHandler inputHandler, TriggerBase trigger)
        {
            ...
            if (binding == null)
            {
                DevdogLogger.LogWarning("Couldn't find binding for keycode : " + inputHandler, this);
                return null;
            }
        ...
        public virtual void Repaint(TriggerBase trigger)
        {
            ...
            if (imageIcon != null && imageIcon.sprite != icon) imageIcon.sprite = icon;

            if (shortcutText != null && shortcutText.text != actionName) shortcutText.text = actionName;
        }
