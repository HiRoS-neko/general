...
namespace Devdog.General.Localization.Editors
{
    ...
    public class LocalizationManagerEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            {
                var path = EditorUtility.SaveFilePanelInProject("Save file", "MainLanguageDb.asset", "asset",
                    "Choose a location where to save the localization database.");
                if (path.EndsWith(".asset"))
                {
                    var indexOf = path.LastIndexOf(Path.DirectorySeparatorChar);
                    if (indexOf == -1) indexOf = path.LastIndexOf('/');
                ...
                if (string.IsNullOrEmpty(path) == false)
                {
                    ...
                    var defaultDb = typeof(LocalizationManager).GetField("_defaultDatabase",
                        BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                    defaultDb.SetValue(t, asset);

                    var databases = typeof(LocalizationManager).GetField("_databases",
                        BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            if (GUILayout.Button("Open Localization Editor")) LocalizationEditorWindow.ShowWindow();
        }
    }
}