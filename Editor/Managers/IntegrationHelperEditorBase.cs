...
namespace Devdog.General.Editors
{
    public abstract class IntegrationHelperEditorBase : EditorWindow
    {
        ...
        protected void ShowIntegration(string name, string description, string link, string defineName,
            bool showBox = true)
        {
            if (showBox)
        ...
        protected List<BuildTargetGroup> GetAllTargets()
        {
            ...
            {
                var l = e.ToString();
                if (l == BuildTargetGroup.Unknown.ToString()) continue;

                var member = t.GetMember(e.ToString()).FirstOrDefault();
                if (member != null)
                {
                    var isObsolete = member.GetCustomAttributes(typeof(ObsoleteAttribute), false).Any();
                    if (isObsolete == false)
        ...
        private static void ReimportRandomScript(string targetFolder)
        {
            ...
            if (targetFolder == null)
                ...
                scripts = AssetDatabase.FindAssets("t:MonoScript", new[] { targetFolder });
        ...
        {
            return "https://www.assetstore.unity3d.com/?asac=ZaKxxhQWGy#!/content/" + ID +
                   "&utm_source=IntegrationWizard";
        }
    }
}