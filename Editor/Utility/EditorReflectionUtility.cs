...
namespace Devdog.General.Editors
{
    public static class EditorReflectionUtility
    {
        ...
        public static object CreateDeepClone(object obj)
        {
            ...
            if (type.IsPrimitive || type.IsEnum || type == typeof(string)) return obj;
            ...
            if (type.IsArray)
            {
                var typeName = type.AssemblyQualifiedName.Replace("[]", string.Empty);
                var typeElement = Type.GetType(typeName);
                if (typeElement != null)
                {
                    var array = obj as Array;
                    var copiedArray = Array.CreateInstance(typeElement, array.Length);
                    for (var i = 0; i < array.Length; i++) copiedArray.SetValue(CreateDeepClone(array.GetValue(i)), i);
                ...
                foreach (var field in fields)
                {
                    ...
                    if (fieldValue != null)
                    {
                        ...
                        if (field.FieldType.IsAssignableFrom(typeof(Object)) ||
                            field.FieldType.IsAssignableFrom(typeof(MonoBehaviour)))
                        {
