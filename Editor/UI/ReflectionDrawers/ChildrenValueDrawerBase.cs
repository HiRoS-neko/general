...
namespace Devdog.General.Editors.ReflectionDrawers
{
    ...
    {
        private readonly bool _hideTypePicker;
            ...
            : base(fieldInfo, value, parentValue, arrayIndex)
        {
            if (fieldInfo != null)
                _hideTypePicker =
                    fieldInfo.GetCustomAttributes(typeof(HideTypePicker), true).Length >
                    0; // Allow by default, unless otherwise specified
        ...
        {
            if (hideGroup) return children.Sum(o => o.GetHeight());
        ...
        {
            if (_hideTypePicker == false) DrawTypeSelector(rect);
            DrawChildren(rect);
        ...
        protected GUIContent GetFoldoutName()
        {
            ...
            if (string.IsNullOrEmpty(childValueName) == false) return new GUIContent(childValueName);
        ...
        protected virtual void DrawTypeSelector(Rect rect)
        {
            ...
            if (t.IsValueType == false)
            {
                ...
                if (required && value == null) GUI.color = Color.red;
                ...
                if (r.Contains(Event.current.mousePosition) && Event.current.type == EventType.Used)
                {
                    ...
                    for (var i = 0; i < implementableTypes.types.Length; i++)
                    {
                        var index = i;
