    ...
    public abstract class DrawerBase : IEquatable<DrawerBase>
    {
        protected DrawerBase(FieldInfo fieldInfo, object value, object parentValue, int arrayIndex)
        {
            ...
            if (fieldInfo != null)
            {
                ...
                headerAttribute =
                    (HeaderAttribute)customAttributes.FirstOrDefault(o => o.GetType() == typeof(HeaderAttribute));
                hideGroupAttribute =
                    (HideGroupAttribute)customAttributes.FirstOrDefault(o => o.GetType() == typeof(HideGroupAttribute));
                onlyDerivedTypesAttribute =
                    (OnlyDerivedTypesAttribute)customAttributes.FirstOrDefault(o =>
                        o.GetType() == typeof(OnlyDerivedTypesAttribute));
            }
        }
        ...
        public Object unityObjectValue => (Object)value;


        public object parentValue { get; protected set; }
        public FieldInfo fieldInfo { get; protected set; }

        public virtual bool isEmpty => value == null || value.Equals(null);
        ...
        public bool hideGroup
        {
            get
            {
                if (hideGroupAttribute == null) return false;

                if (isInArray) return hideGroupAttribute.includeArrayChildren;
        ...
        public virtual GUIContent fieldName
        {
            get
            {
                if (overrideFieldName != null) return overrideFieldName;

                var lengthText = "";
                var arr = value as Array;
                if (arr != null) lengthText = " : " + arr.Length;

                if (isInArray) return new GUIContent("Element " + arrayIndex);

                if (fieldInfo == null)
                    return new GUIContent(ObjectNames.NicifyVariableName(value.GetType().Name) + lengthText);

                return new GUIContent(ObjectNames.NicifyVariableName(fieldInfo.Name) + lengthText);
            }
        }


        public bool isInArray => arrayIndex >= 0;
        ...
        public bool Equals(DrawerBase other)
        {
            ...
            return ReferenceEquals(value, other.value) && arrayIndex == other.arrayIndex;
        }
        ...
        public string GetFieldTypeName(bool getLowestType)
        {
            ...
            if (fieldType.IsGenericType)
            {
                if (isInArray)
                {
                    ...
                    if (v == null || getLowestType) return ReflectionDrawerUtility.GetGenericTypeNiceName(fieldType);

                    if (v.GetType().IsGenericType) return ReflectionDrawerUtility.GetGenericTypeNiceName(v.GetType());
        ...
        public Type GetFieldType(bool getLowestType)
        {
            if (isInArray)
            {
                ...
                // Couldn't determine type, going with array's type.
                if (v == null || getLowestType) return fieldInfo.FieldType.GetElementType();
...
//            }
            ...
            if ((getLowestType || value == null) && fieldInfo != null) return fieldInfo.FieldType;
        ...
        {
            if (headerAttribute != null && isInArray == false) return ReflectionDrawerStyles.headerSize;
        ...
        public void Draw(ref Rect rect)
        {
            ...
            if (required && isEmpty) GUI.color = Color.red;
            ...
            {
                if (EditorGUI.EndChangeCheck()) NotifyValueChanged(val);
            }
        ...
        public virtual void NotifyValueChanged(object newValue)
        {
            ...
            {
                if (fieldInfo != null) fieldInfo.SetValue(parentValue, newValue);
            }
