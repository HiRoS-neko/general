...
namespace Devdog.General.Editors.ReflectionDrawers
{
    ...
    {
        private readonly ArrayControlOptionsAttribute _arrayControlOptionsAttribute;

        public ArrayDrawer(FieldInfo fieldInfo, object value, object parentValue, int arrayIndex)
            : base(fieldInfo, value, parentValue, arrayIndex)
        {
            if (fieldInfo != null)
                _arrayControlOptionsAttribute = (ArrayControlOptionsAttribute)fieldInfo
                    .GetCustomAttributes(typeof(ArrayControlOptionsAttribute), true).FirstOrDefault();
        ...
        public bool drawAddButton
        {
            get
            {
                if (_arrayControlOptionsAttribute == null) return true;
        ...
        {
            var add = 0;
            if (drawAddButton) add += ReflectionDrawerStyles.singleLineHeight;

            if (hideGroup) return children.Sum(o => o.GetHeight()) + add;
        ...
        protected override object DrawInternal(Rect rect)
        {
            if (hideGroup)
            {
                ...
                if (drawAddButton) DrawArrayAddButton(rect);
            }
            else
            {
                using (var foldout = new FoldoutBlock(this, rect, new GUIContent(fieldName)))
                {
                    ...
                    if (foldout.isUnFolded)
                        using (var indent = new IndentBlock(rect))
                        {
                            ...
                            if (drawAddButton) DrawArrayAddButton(indent.rect);
                        }
                }
            }
        ...
        {
            if (drawRemoveButton) rect.width -= 20f;

            var elementRemoved = -1;
        ...
        private void DrawArrayAddButton(Rect rect)
        {
            ...
            {
                var derivedTypeInfo = ReflectionDrawerUtility.GetDerivedTypesFrom(fieldInfo.FieldType.GetElementType(),
                    onlyDerivedTypesAttribute != null ? onlyDerivedTypesAttribute.type : null);

                var arrFieldType = fieldInfo.FieldType.GetElementType();
                if (onlyDerivedTypesAttribute != null) arrFieldType = onlyDerivedTypesAttribute.type;

                rect.width -= 25;
                if (rect.Contains(Event.current.mousePosition) && arrFieldType.IsAbstract == false &&
                    arrFieldType.IsInterface == false &&
                    ReflectionDrawerUtility.IsGenericTypeWithoutGenericArguments(arrFieldType) == false)
                {
                    // Clicked left side
                ...
                else
                {
                    ...
                    for (var i = 0; i < derivedTypeInfo.types.Length; i++)
...
//            Debug.Log("Add array element");
            ...
            for (var j = 0; j < newArray.Length - 1; j++) newArray.SetValue(arrayValue.GetValue(j), j);

            object newValue = null;

            // Can't construct UnityEngine.Object values
            if (typeof(Object).IsAssignableFrom(createOfType) == false)
            {
...
//            Debug.Log("Removing array element " + i);
            var newArray = Array.CreateInstance(fieldInfo.FieldType.GetElementType(), arrayValue.Length - 1);
            for (var j = 0; j < i; j++) newArray.SetValue(arrayValue.GetValue(j), j);
            for (var j = i; j < arrayValue.Length - 1; j++)
