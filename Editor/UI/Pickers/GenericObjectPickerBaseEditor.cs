...
namespace Devdog.General.Editors
{
    public abstract class GenericObjectPickerBaseEditor<T> : BetterEditorWindow where T : class
    {
        ...
        public T selectedObject => objects[selectedIndex];

        public bool isUtility { get; protected set; }

        public bool isSearching => string.IsNullOrEmpty(searchQuery) == false && searchQuery != "Search...";
        ...
        {
            if (OnPickObject != null) OnPickObject(obj);
        ...
        protected override void OnKeyUp(KeyCode keyCode)
        {
            if (keyCode == KeyCode.DownArrow)
                selectedIndex++;
            else if (keyCode == KeyCode.UpArrow) selectedIndex--;
        ...
        {
            if (GUILayout.Button(obj.ToString())) NotifyPickedObject(obj);
        }
        ...
        protected virtual void ShowInfoBox()
        {
            EditorGUILayout.HelpBox(
                "Use the up and down arrow keys to select an item.\nHit enter to pick the highlighted item.",
                MessageType.Info);
        }
    }
}