...
//        private static Dictionary<Rect, int> _unityObjectPickerControlID = new Dictionary<Rect, int>();
        private static readonly Dictionary<Type, Type> _objectPickerTypeCache = new();
        private static readonly Type[] _allEditorTypes = new Type[0];
        private static readonly Dictionary<Type, CustomObjectPickerAttribute> _allEditorTypeAttributes = new();
        ...
        static ObjectPickerUtility()
        {
            ...
            foreach (var type in _allEditorTypes)
                if (typeof(ObjectPickerBaseEditor).IsAssignableFrom(type) == false)
                    DevdogLogger.LogError("Class " + type.Name + " has the " +
                                          typeof(CustomObjectPickerAttribute).Name +
                                          " attribute, but doesn't inherit from " +
                                          typeof(ObjectPickerBaseEditor).Name);

            _allEditorTypes = _allEditorTypes.OrderByDescending(o =>
                ((CustomObjectPickerAttribute)o.GetCustomAttributes(typeof(CustomObjectPickerAttribute), true).First())
                .priority).ToArray();
            foreach (var type in _allEditorTypes)
                _allEditorTypeAttributes[type] =
                    (CustomObjectPickerAttribute)type.GetCustomAttributes(typeof(CustomObjectPickerAttribute), true)
                        .First();
        }
        ...
        private static Type builtInObjectPickerType
        {
            get
            {
                ...
                if (type == null) DevdogLogger.LogError("Built-in Unity ObjectSelector not found!");
        ...
        private static bool isUnityObjectPickerOpen
        {
            ...
            {
                var isVisibleField =
                    builtInObjectPickerType.GetProperty("isVisible", BindingFlags.Public | BindingFlags.Static);
                if (isVisibleField == null)
                {
                    DevdogLogger.LogError(
                        "Couldn't find isVisible field on UnityEditor.ObjectSelector's internal class");
                    return false;
                }
        ...
        public static ObjectPickerBaseEditor GetObjectPickerForType(Type objectType, Action<Object> callback)
        {
            var editorType = GetObjectPickerTypeFor(objectType);

            var window = (ObjectPickerBaseEditor)EditorWindow.GetWindow(editorType, true);
            window.searchType = typeof(Component).IsAssignableFrom(objectType)
                ? ObjectPickerBaseEditor.SearchType.Components
                : ObjectPickerBaseEditor.SearchType.ObjectTypes;
        ...
        {
            if (_objectPickerTypeCache.ContainsKey(objectType)) return _objectPickerTypeCache[objectType];
            ...
            where T : Object
        {
            ...
            RenderObjectPickerForType(prop.displayName, prop, callback);
        }
        ...
        {
            Assert.IsNotNull(prop, "Given SerializedProperty is null!");
            RenderObjectPickerForType(rect, prop.displayName, prop, callback);
        }

        public static void RenderObjectPickerForType<T>(string fieldName, SerializedProperty prop,
            Action<T> callback = null)
            where T : Object
        {
            RenderObjectPickerForType(fieldName, prop, typeof(T), val =>
            {
                if (callback != null) callback((T)val);
            });
        }

        public static void RenderObjectPickerForType<T>(Rect rect, string fieldName, SerializedProperty prop,
            Action<T> callback = null)
            where T : Object
        {
        ...
        public static void RenderObjectPickerForType(string fieldName, SerializedProperty prop, Type type,
            Action<Object> callback = null)
        {
            var rect = GUILayoutUtility.GetRect(GetGUIContentFromSerializedProperty(prop, type),
                UnityEditor.EditorStyles.objectField);
            RenderObjectPickerForType(rect, fieldName, prop, type, callback);
        }

        public static void RenderObjectPickerForType(Rect rect, string fieldName, SerializedProperty prop, Type type,
            Action<Object> callback = null)
        {
            Assert.IsNotNull(prop, "Given SerializedProperty is null!");
            ...
            {
                RenderObjectPickerForType(rect, type, new GUIContent(fieldName),
                    GetGUIContentFromSerializedProperty(prop, type), prop.objectReferenceValue, val =>
                    {
        ...
        public static void RenderObjectPickerForType<T>(string fieldName, T prop, Action<T> callback = null)
            where T : Object
        {
            var rect = GUILayoutUtility.GetRect(GetGUIContentFromUnityObject(prop, typeof(T)),
                UnityEditor.EditorStyles.objectField);
            RenderObjectPickerForType(rect, fieldName, prop, callback);
        }

        public static void RenderObjectPickerForType<T>(Rect rect, string fieldName, T prop, Action<T> callback = null)
            where T : Object
        {
            RenderObjectPickerForType(rect, typeof(T), new GUIContent(fieldName),
                GetGUIContentFromUnityObject(prop, typeof(T)), prop, obj =>
                {
                    if (callback != null) callback((T)obj);
                });
        }

        public static void RenderObjectPickerForType(string fieldName, Object obj, Type type,
            Action<Object> callback = null)
        {
            var rect = GUILayoutUtility.GetRect(GetGUIContentFromUnityObject(obj, type),
                UnityEditor.EditorStyles.objectField);
            RenderObjectPickerForType(rect, type, new GUIContent(fieldName), GetGUIContentFromUnityObject(obj, type),
                obj, callback);
        }

        public static void RenderObjectPickerForType(Rect rect, string fieldName, Object prop, Type type,
            Action<Object> callback = null)
        {
            RenderObjectPickerForType(rect, type, new GUIContent(fieldName), GetGUIContentFromUnityObject(prop, type),
                prop, callback);
        }

        private static void RenderObjectPickerForType(Rect rect, Type type, GUIContent fieldName, GUIContent objectName,
            Object obj, Action<Object> callback = null)
        {
            DoObjectField(rect, rect, fieldName, 1, obj, type, callback);
        }

        internal static void DoObjectField(Rect position, Rect dropRect, GUIContent fieldName, int id, Object obj,
            Type objType, Action<Object> callback)
        {
            ...
            if (!GUI.enabled && Event.current.rawType == EventType.MouseDown) eventType = Event.current.rawType;
            ...
            switch (eventType2)
            {
                ...
                case EventType.DragPerform:
                    if (dropRect.Contains(Event.current.mousePosition) && GUI.enabled)
                    {
                        ...
                        if (object2 != null && !UnityEditor.EditorUtility.IsPersistent(object2)) object2 = null;

                        if (object2 != null)
                        {
                            DragAndDrop.visualMode = DragAndDropVisualMode.Generic;
                            if (eventType == EventType.DragPerform &&
                                ValidateObjectFieldAssignment(new[] { object2 }, objType) != null)
                            {
                                obj = object2;
                                GUI.changed = true;

                                var objAsGameObject = obj as GameObject;
                                if (objAsGameObject != null) obj = objAsGameObject.GetComponent(objType);
        ...
        private static void DrawObjectFieldCustom(Rect position, int id, Object obj, Type objType,
        ...
        internal static Object ValidateObjectFieldAssignment(Object[] references, Type objType)
        {
            if (references.Length > 0)
            {
                ...
                {
                    var gameObject = (GameObject)references[0];
                ...
                {
                    var @object = array[i];
                    if (@object != null && objType.IsAssignableFrom(@object.GetType())) return @object;
                }
            }
        ...
        private static GUIContent GetGUIContentFromUnityObject(Object obj, Type type)
        {
