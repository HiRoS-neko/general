...
{
    public abstract class EditorCrudBase<T> : IEditorCrud where T : class
    {
        private readonly Color _createColor = new(0.4f, 1.0f, 0.4f, 0.8f);
        ...
        public bool isSearching => searchQuery != "" && searchQuery != "Search...";
        ...
        {
            if (forceUpdateIDsWhenOutOfSync && crudList.Count > 0 && IDsOutOfSync()) SyncIDs();
            ...
            if (selectedItem != null) DrawDetail(selectedItem, crudList.FindIndex(o => o.Equals(selectedItem)));
        ...
        {
            return EditorReflectionUtility.CreateDeepClone(crudList[index]);
        }
        ...
        {
            var rect = EditorGUILayout.GetControlRect(true, 30.0f);
            rect.x -= 7;
            rect.width = 260;
        ...
        protected virtual bool DrawSidebarRowElementButton(string text, int width, int height = -1)
        {
            sidebarRowElementOffset.width = width;
            sidebarRowElementOffset.height = height == -1 ? sidebarRowElementOffset.height : height;
            var clicked = GUI.Button(sidebarRowElementOffset, text);
        ...
        protected virtual bool DrawSidebarRowElementButton(string text, GUIStyle guiStyle, int width, int height = -1)
        {
            sidebarRowElementOffset.width = width;
            sidebarRowElementOffset.height = height == -1 ? sidebarRowElementOffset.height : height;
            var clicked = GUI.Button(sidebarRowElementOffset, text, guiStyle);
        ...
        protected virtual bool DrawSidebarRowElementToggle(bool toggled, string text, int width, int height = -1)
        {
            sidebarRowElementOffset.width = width;
            sidebarRowElementOffset.height = height == -1 ? sidebarRowElementOffset.height : height;
            var t = GUI.Toggle(sidebarRowElementOffset, toggled, text);

            sidebarRowElementOffset.x += width;
            return t;
        }

        protected virtual bool DrawSidebarRowElementToggle(bool toggled, string text, GUIStyle guiStyle, int width,
            int height = -1)
        {
            sidebarRowElementOffset.width = width;
            sidebarRowElementOffset.height = height == -1 ? sidebarRowElementOffset.height : height;
            var t = GUI.Toggle(sidebarRowElementOffset, toggled, text, guiStyle);
        ...
        protected virtual void EndSidebarRow(T item, int i)
        {
            ...
            if (canDuplicateItems)
            {
                ...
                if (GUI.Button(sidebarRowElementOffset, "", "OL Plus")) DuplicateItem(i);
            ...
            {
                GUI.color = Color.red;
                if (GUI.Button(sidebarRowElementOffset, "", "OL Minus"))
                    if (UnityEditor.EditorUtility.DisplayDialog("Are you sure?", "Do you want to delete " + singleName,
                            "Yes", "NO!"))
                        RemoveItem(i);
            }
        ...
        protected virtual void DrawSidebar()
        {
            ...
            if (hideCreateItem == false)
            {
                ...
                if (GUILayout.Button("Create " + singleName, "LargeButton"))
                    CreateNewItem();
            ...
            if (isSearching)
                GUILayout.Label(_searchResultCount + " " + pluralName +
                                " (search result)"); // , InventoryEditorStyles.titleStyle
            else
            ...
            var searchResultIndexItem = -1;
            var nullInList = false;
