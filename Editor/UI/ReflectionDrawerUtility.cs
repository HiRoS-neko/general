...
namespace Devdog.General.Editors.ReflectionDrawers
{
    ...
    {
        private static readonly Dictionary<Type, Type> _drawers = new()
        {
        ...
        private static readonly Dictionary<Type, CustomDrawerAttribute> _customDrawerAttributes;
        private static readonly Dictionary<Type, List<FieldInfo>> _serializableFieldsCache = new();
        ...
        static ReflectionDrawerUtility()
        {
            ...
            foreach (var c in customDrawerClasses)
                _customDrawerAttributes[c] =
                    (CustomDrawerAttribute)c.GetCustomAttributes(typeof(CustomDrawerAttribute), true).FirstOrDefault();
        }
        ...
        public static DrawerBase BuildEditorHierarchy(FieldInfo field, object value, int index = -1)
        {
            ...
            {
                // Is not an array
                if (value != null) fieldValue = field.GetValue(value);
            }

            Type drawerType;
            var found = _drawers.TryGetValue(fieldType, out drawerType);
            if (found == false) drawerType = TryGetCustomDrawerType(fieldType, field == null);
            ...
            if (drawerType == null)
            {
                if (fieldType.IsEnum)
                    ...
                    drawerType = typeof(DictionaryDrawer);
                         ...
                         (value.GetType().IsArray && value.GetType().GetElementType().IsInterface))
                    drawerType = typeof(InterfaceDrawer);
                else if (fieldType.IsClass)
                    drawerType = typeof(ClassDrawer);
                else if (fieldType.IsValueType) drawerType = typeof(ValueTypeDrawer);

                // Cache it for future usage.
                if (drawerType != null) _drawers[fieldType] = drawerType;
            }
        ...
        public static Type TryGetCustomDrawerType(Type forType, bool isRootType)
        {
            ...
            {
                if (c.Value.onlyForRoot && isRootType == false) continue;
        ...
        public static DrawerBase TryGetCustomDrawer(FieldInfo field, object fieldValue, object parentValue, int index,
            Type fieldType)
        {
        ...
        private static DrawerBase CreateNewDrawerFromType(FieldInfo field, object fieldValue, object parentValue,
            int index, Type drawerType)
        {
            ...
            return (DrawerBase)constructor.Invoke(new[] { field, fieldValue, parentValue, index });
        }

        public static DerivedTypeInformation GetDerivedTypesFrom(Type fromType, Type onlyDerivedFrom)
        {
            if (fromType.IsGenericType) fromType = fromType.GetGenericTypeDefinition();

            if (onlyDerivedFrom != null) fromType = onlyDerivedFrom;
            ...
            if (fromType.IsGenericType)
            {
                ...
                implementable.content =
                    implementable.types.Select(o => new GUIContent(GetGenericTypeNiceName(o))).ToArray();
            }
            else
            {
                implementable.types = ReflectionUtility.GetAllTypesThatImplement(fromType);
                implementable.content =
                    implementable.types.Select(o => new GUIContent(GetPopupNameForType(o))).ToArray();
            }

            return implementable;
        }

        private static Type[] TryAddTypes(Type fromType)
        {
        ...
        public static string GetGenericTypeNiceName(Type type)
        {
            ...
            var names = type.GetGenericArguments().Select(o => o.Name).ToArray();
        ...
        public static string GetPopupNameForType(Type type)
        {
            ...
            var name = "";
            foreach (var s in type.Namespace.Split('.')) name += s + "/";
