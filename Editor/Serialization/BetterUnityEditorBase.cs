...
namespace Devdog.General.Editors
{
    public class BetterUnityEditorBase : Editor
    {
        ...
        private List<DrawerBase> GetMainDrawers()
        {
            ...
            var editor =
                ReflectionDrawerUtility.TryGetCustomDrawer(null, target, null, -1, target.GetType()) as
                    ChildrenValueDrawerBase;
            if (editor != null)
            {
        ...
        {
            if (mainDrawers == null) mainDrawers = GetMainDrawers();


            var width = Mathf.Min(EditorGUIUtility.currentViewWidth, 800f) - 50;
            var drawersHeight = mainDrawers.Sum(o => o.GetHeight());

            var r = GUILayoutUtility.GetRect(300, width, drawersHeight + 10, drawersHeight + 10,
            ...
            foreach (var drawer in mainDrawers) drawer.Draw(ref drawerRect);

            if (EditorGUI.EndChangeCheck()) UnityEditor.EditorUtility.SetDirty(target);
