...
namespace Devdog.General.Localization.Editors
{
    ...
    public class LocalizedStringDrawer : SimpleValueDrawer
    {
        ...
        {
            _textArea = (TextAreaAttribute)fieldInfo.GetCustomAttributes(typeof(TextAreaAttribute), true)
                .FirstOrDefault();
        ...
        public override bool isEmpty => IsNullOrWhiteSpace(((LocalizedString)value).message);

        protected override int GetHeightInternal()
        {
            if (_textArea != null)
                return ReflectionDrawerStyles.singleLineHeight * 2 +
                       ReflectionDrawerStyles.singleLineHeight * Mathf.Max(_textArea.maxLines, 3);
        ...
        protected override object DrawInternal(Rect rect)
        {
            ...
            if (_textArea != null)
            {
                ...
                LocalizationEditorUtility.DrawLanguagePicker(ref rect, keyName,
                    newKeyName => { _keyField.SetValue(value, newKeyName); });
                ...
                if (LocalizationManager.currentDatabase != null &&
                    LocalizationManager.currentDatabase.ContainsString(keyName) == false)
                    GUI.color = new Color(1f, 1f, 1f, 0.5f);

                var str2 = EditorGUI.TextArea(rect, a.message ?? "");
                if (EditorGUI.EndChangeCheck() && LocalizationManager.defaultDatabase != null &&
                    str2 != LocalizationManager.defaultDatabase.GetString(keyName))
                {
            ...
            if (EditorGUI.EndChangeCheck() && LocalizationManager.defaultDatabase != null &&
                str != LocalizationManager.defaultDatabase.GetString(keyName))
            {
                if (IsNullOrWhiteSpace(keyName) || keyName == LocalizationManager.NoKeyConstant)
                {
                    _keyField.SetValue(value, LocalizationManager.CreateNewStringKey());
        ...
        private void Update()
        {
            ...
            if (typeof(Object).IsAssignableFrom(fieldType) &&
                ReflectionDrawerUtility.TryGetCustomDrawerType(fieldType, fieldInfo == null) == null) return;
