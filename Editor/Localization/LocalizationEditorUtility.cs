...
namespace Devdog.General.Localization.Editors
{
    ...
    {
        private static readonly GUIStyle _toolbarButtonStyle = "toolbarbutton";


        private static FieldInfo _GetLocalizedObjectKey(object str)
        {
            return str.GetType().GetField("_key",
...
//        }
        ...
        public static void DrawLanguagePicker(ref Rect rect, string key, Action<string> keyRenameCallback)
        {
            ...
            var shownLanguages = dbs.Where(o => o != null).Select(o => new GUIContent(o.lang))
                .Take((int)Mathf.Max(3f, rect.width / 60f)).ToArray();
            if (shownLanguages.Length > 0)
            {
                EditorGUI.BeginChangeCheck();
                var index = GetCurrentLanguageIndex(shownLanguages, LocalizationManager.currentLanguage);
            ...
            if (GUI.Button(r, "...", _toolbarButtonStyle))
            {
                ...
                foreach (var lang in dbs)
                    menu.AddItem(new GUIContent("Languages/" + lang.lang), LocalizationManager.currentDatabase == lang,
                        db => { LocalizationManager.SetLanguage((LocalizationDatabase)db); }, lang);
